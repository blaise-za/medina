...
while IFS=',' read -r codemms name firstname genre class ranking student_nb average year_average recipient file_path is_done other
  do
    ...
    export file_path=$( echo $base_path/"$code_mms"_BS2.jpeg)
    export filename=$( echo "$code_mms"_BS2.jpeg)
    ...
    if [[ $is_done == 'no' && $does_exist == '1' ]]; then
      j2 --format=env message-rentree-2025.txt.jinja2 > message.txt
