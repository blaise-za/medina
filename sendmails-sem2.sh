...
while IFS=',' read -r codemms firstname name class ranking student_nb average year_average recipient file_path filename from cc genre is_done other
  do
    ...
    if [[ $is_done == 'no' && $does_exist == '1' ]]; then
      j2 --format=env message-sem2.txt.jinja2 > message.txt
